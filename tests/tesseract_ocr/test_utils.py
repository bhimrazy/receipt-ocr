...
def test_perform_ocr_no_receipt_outline(mock_imutils, mock_pytesseract, mock_cv2):
    ...
    mock_imutils.grab_contours.return_value = [np.array([[[0,0],[1,1],[2,2]]], dtype=np.int32)] # A contour not of 4 points
    mock_cv2.contourArea.return_value = 1000
    mock_cv2.arcLength.return_value = 200
    mock_cv2.approxPolyDP.return_value = np.array([[[0,0],[1,1],[2,2]]], dtype=np.int32) # Not 4 points
